...
Generate a nearest query PolygonPoint and Value's Indexable relationship while calculating
distances. This function may be used to define that knn query should calculate distances
as smallest as possible between query PolygonPoint and Indexable's points. In other words it
...
\tparam T   Type of wrapped object. This may be a PolygonPoint for PointRelation or CoordinateType for
            MinRelation or MaxRelation

\param v    PolygonPoint or distance value.
*/
template <typename T>
...
Generate a nearest query PolygonPoint and Value's Indexable relationship while calculating
distances. This function may be used to define that knn query should calculate distances
between query PolygonPoint and Indexable's centroid. This function may be also used
to define distances bounds which indicates that Indexable's centroid should be
closer or further than value v.

\ingroup nearest_relations

\tparam T   Type of wrapped object. This may be a PolygonPoint for PointRelation or some CoordinateType for
            MinRelation or MaxRelation
...
Generate a nearest query PolygonPoint and Value's Indexable relationship while calculating
distances. This function may be used to define that knn query should calculate distances
as biggest as possible between query PolygonPoint and Indexable's points. In other words it
...
if distances between PolygonPoint and Indexable are the smallest. Distance calculation is defined
...
\param pr               The point relation. This may be generated by \c index::to_nearest(),
                        \c index::to_centroid() or \c index::to_furthest() with \c PolygonPoint passed as a parameter.
*/
...
Generate a distance predicate. This defines distances bounds which are used by knn query.
This function indicates that Values should be returned only if distances between PolygonPoint and
...
PolygonPoint but only if nearest points are further than some distance.
...
\param pr               The point relation. This may be generated by \c to_nearest(),
                        \c to_centroid() or \c to_furthest() with \c PolygonPoint passed as a parameter.
...
defined by PointRelation. So it is possible e.g. to return Values with centroids closest to some
PolygonPoint but only if nearest points are closer than some distance.
...
So it is possible e.g. to return Values with centroids closest to some PolygonPoint but only if nearest
